...
class Logger(object):
    ...
    def print_statistics(self, run=None, mode='max_acc', epoch=100):
        if run is not None:
            ...
            self.best_epoch_list.append(ind+1)
        else:
            result = [100 * r for r in self.results]
            best_results = []
            for r in result:
            ...
            print(f'   Chosen Epoch: {np.array(self.best_epoch_list).mean():.2f}±{np.array(self.best_epoch_list).std():.2f}')
